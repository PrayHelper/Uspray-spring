...
public class OAuth2LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        if(customOAuth2User.getAuthority() == Authority.ROLE_GUEST) {
            ...
            response.sendRedirect("http://localhost:3000/socialLoginNameInput"+"?token="+accessToken+"&id="+id.substring(4, id.length()-1)); // 프론트의 회원가입 추가 정보 입력 폼으로 리다이렉트 (추가 컨트롤러를 만들고 거기서 post 해야지 User로 바뀜)
            return;
        }
